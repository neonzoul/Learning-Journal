...
# Calling when calling with python file like test_db.py (if not call with main it will not run)
# [[ meaning if just import this function will not run regardless of whether other functions from the module are called. ]]
if __name__ == "__main__":
    main()

"""
For example:
1. If another file does `import test_db`, the `main()` function won't run
2. Even if that file calls other functions like `test_db.create_db_and_tables()`, the `main()` function still won't run
3. The only way to run `main()` when importing would be to explicitly call `test_db.main()`

This pattern effectively makes the `main()` function optional when the module is imported, allowing you to use the other functionality without triggering the full database creation and population process.
"""